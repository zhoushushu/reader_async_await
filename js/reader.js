...
async function fetchdata () {
  try {
    let step1val = await step1()
    let step2val = await step2(step1val)
    let step3val = await step3(step2val)
    chapterContentRender(step3val)
  } catch (error) {
    window.alert(error)
  }
}
// 上一页、下一页数据
async function fetchnextpagedata (chapterId) {
  try {
    let step2val = await step2(chapterId)
    let step3val = await step3(step2val)
    chapterContentRender(step3val)
  } catch (error) {
    window.alert(error)
